...
# ============================================================
# 🔶 LIVE AGENTIC INFERENCE & XAI (FINAL STABLE VERSION)
# ============================================================
st.divider()
st.header("🎮 Manual Agentic Diagnostic Tool")

# Global feature list used for consistency across the app
FEATURES_LIST = ['PM2.5', 'PM10', 'NO2', 'SO2', 'CO', 'Ozone']

# 1. Create the Input Form
with st.form("agent_input_form"):
    st.subheader("📝 Enter Real-Time Sensor Data")
    st.write("Tip: You can click inside the box and type numbers directly.")
    ...
    for i, feature in enumerate(FEATURES_LIST):
        with [col1, col2, col3][i % 3]:
            # Get median from data if available, else use a safe default
            if 'data' in locals() and feature in data.columns:
                default_val = float(data[feature].median())
            else:
                default_val = 20.0  # Safe generic default

            # format="%.2f" and step=1.0 allow for direct typing and easy adjustment
            val = st.number_input(
                f"Enter {feature}", 
                value=default_val, 
                step=1.0, 
    ...
    submit_btn = st.form_submit_button("🚀 Run Agentic Analysis")

# 2. Process Input on Click
if submit_btn:
    # Check if the pipeline (scaler and registry) exists
    if 'registry_agent' in locals() and 'scaler' in locals():
        try:
            # Prepare and Scale Input
            raw_values = np.array([user_inputs[f] for f in FEATURES_LIST]).reshape(1, -1)
            scaled_values = scaler.transform(raw_values)
            
            # Identify Best Model
            best_name, best_info = registry_agent.best_model()
            model = best_info["model"]
            
            # Predict Category
            pred_idx = int(model.predict(scaled_values)[0])
            
            # Create Display Columns
            res_col1, res_col2 = st.columns([1, 2])
            
            with res_col1:
                st.metric("Predicted Category", f"Class {pred_idx}")
                # Agentic Recommendation based on Pollutant Driver
                advice = rec_agent.recommend(pred_idx, FEATURES_LIST, list(user_inputs.values()))
                st.info(advice)
                
            with res_col2:
                st.subheader("🔍 Local XAI Explanation")
                # Force SHAP to explain the specific user input
                explainer = shap.TreeExplainer(model)
                shap_values = explainer.shap_values(scaled_values)
                
                # Logic to handle multi-class SHAP shapes
                if isinstance(shap_values, list):
                    current_shap = shap_values[pred_idx][0]
                else:
                    # For some models, SHAP returns (samples, features, classes)
                    current_shap = shap_values[0] if len(shap_values.shape) < 3 else shap_values[0, :, pred_idx]

                shap_df = pd.DataFrame({
                    "Feature": FEATURES_LIST,
                    "Influence": current_shap
                }).sort_values(by="Influence", ascending=True)
                
                fig_shap = px.bar(
                    shap_df, x="Influence", y="Feature", orientation='h',
                    title=f"Feature Impact for Class {pred_idx}",
        ...
        st.warning("⚠️ Pipeline Not Ready: Please ensure Patch 1 and Patch 2 have finished running.")
# ============================================================
# 🔶 FINAL STATUS
# ============================================================
